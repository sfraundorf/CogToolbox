function SubtractionDistractorTask(monitor, rect, fontSize, fontColor, backColor, duration, ISI, filename)
%
% SubtractionDistractorTask(monitor, rect, fontSize, fontColor, backColor, duration, ISI, filename)
%
% Presents 2-digit subtraction problems to the screen until at least DURATION
...
if nargin < 8
    ...
    try
       mathout=fopen(filename,'w');
        ...
        Screen('CloseAll');
        error('CogToolbox:SubtractionDistractorTask:InvalidFileName', ...
            'File name in subtraction distractor task not valid')
    end
end
...
while true
    ...
    nums=sort(nums); % larger number must come first
